const ProductList = ({ products, deleteProduct }) => {
  return (
    <section className="flex flex-col gap-2 flex-1 mb-20">
      ...
      <div className="w-full flex flex-col gap-y-4 pt-3 px-2 overflow-auto h-[500px]">
        {!products.length ? (
          <span className="text-center text-slate-300">not product's yet</span>
        ) : (
          <>
                ...
                <div className="flex items-center gap-x-4">
                  ...
                  <span
                    className={` h-8 flex items-center justify-center p-2 ${
                      product.selectCategorie.length > 20
                        ? "text-xs "
                        : "text-sm"
                    } rounded-2xl ring-1 ring-slate-300`}
                  >
